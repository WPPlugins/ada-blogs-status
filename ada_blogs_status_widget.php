...
Plugin URI: http://1uthavi.adadaa.com/new-updated-active-blogs-widgets/
...
<div id="ada-multi-sidebar" class="clearfix">
  ...
  <?php	
		...
		case 'updated':
			...
			$order = 'updated';
		?>
  <div id="s-updated" class="widgetcontainer clearfix tabcontent">
    <ul>
  ...
  <div id="s-new" class="widgetcontainer clearfix tabcontent">
    <ul>
      <?php ada_get_most_blogs(get_blog_list(0,$limit,false),$show_desc, $show_avatar, $avatar_size, $tab); ?>
    </ul>
  </div>
  <!-- #s-new -->
  <?php
...
<?php	
	...
	function ada_render_sidebar_tabs($order) {
		...
		foreach ($order as $t) { ?>
<?php if ( $t != 'none' ) : ?>
<li><a href="#s-<?php echo $t; ?>" <?php if($count==0) echo 'class="defaulttab"'; ?> rel="s-<?php echo $t; ?>"><span><?php echo $list[$t]; ?></span></a></li>
<?php endif ?>
<?php $count++; }
	}
